...
func TestDissolve_NoSharedSegments(t *testing.T) {
	a := ls(geom.XY{0, 0}, geom.XY{1, 0})
	b := ls(geom.XY{2, 0}, geom.XY{3, 0})
...
func TestDissolve_IsolatedRing(t *testing.T) {
	...
	a := ls(geom.XY{0, 0}, geom.XY{1, 0}, geom.XY{1, 1}, geom.XY{0, 1}, geom.XY{0, 0})
...
func TestDissolve_PolygonBoundary(t *testing.T) {
	...
	left := geom.NewPolygon(nil, []geom.XY{{0, 0}, {1, 0}, {1, 1}, {0, 1}, {0, 0}})
	right := geom.NewPolygon(nil, []geom.XY{{1, 0}, {2, 0}, {2, 1}, {1, 1}, {1, 0}})
