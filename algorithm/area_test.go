...
func TestAreaGeometryTypes(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "LineString",
			geom:     mustLineStringXY(0, 0, 10, 0),
			expected: 0,
		},
		{
			name:     "Polygon",
		...
		},
		{
			...
			geom: geom.NewMultiPolygon([]*geom.Polygon{
				...
				geom.NewPolygon(mustLinearRingXY(10, 10, 15, 10, 15, 15, 10, 15, 10, 10), nil),
			}),
			expected: 50,
		},
		{
...
func TestLineLength(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "ThreePoints",
			coords:   mustCoordsXY(0, 0, 3, 4, 3, 8),
			expected: 9,
		},
	}
...
func TestLineCentroid(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "ThreePoints",
			coords:    mustCoordsXY(0, 0, 10, 0, 10, 10),
			expectedX: 7.5,
			expectedY: 2.5,
		},
	}
...
func TestRingCentroid(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "Triangle",
			coords:    mustCoordsXY(0, 0, 10, 0, 5, 10, 0, 0),
			expectedX: 5,
			expectedY: 10.0 / 3,
		},
		{
	...
	degenerateRing := mustCoordsXY(0, 0, 10, 0, 0, 0, 0, 0)
...
func TestRingCentroid_OpenRing(t *testing.T) {
	open := mustCoordsXY(0, 0, 10, 0, 10, 10, 0, 10)
	closed := mustCoordsXY(0, 0, 10, 0, 10, 10, 0, 10, 0, 0)
...
func TestRingCentroid_NearZeroAreaFallback(t *testing.T) {
	...
	thin := mustCoordsXY(0, 0, 1, 0, 2, eps, 0, 0)
...
func TestPolygonCentroid(t *testing.T) {
	...
	hole2 := mustLinearRingXY(0, 0, 10, 0, 10, 10, 0, 10, 0, 0)
