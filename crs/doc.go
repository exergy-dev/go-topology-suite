...
//
// Every go-topology-suite geometry carries a *CRS pointer. Operations that mix
// geometries with incompatible CRS return ErrCRSMismatch (defined in the
// top-level gts package) rather than silently producing nonsense; users
// must call gts.Transform explicitly to reconcile. The crs package
// itself supplies the underlying Operation graph (OperationFor) and the
// projection / datum primitives; gts.Transform is the geometry-aware
// wrapper.
//
